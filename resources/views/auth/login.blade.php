						...
						<div class="login-signin">
							...
							<form class="form" action="{{ route('login') }}" method="POST">
								...
								<div class="form-group text-center mt-10">
									<button type="submit" class="btn btn-pill btn-outline-white font-weight-bold opacity-90 px-15 py-3">{{ t('Sign In') }}</button>
								</div>
							</form>
							<div class="mt-10">
								<span class="opacity-70 mr-4">{{ t("Dont have an account yet?") }}</span>
								<a href="{{ route('register') }}" i class="text-white font-weight-bold">{{ t('Sign Up') }}</a>
							</div>
						</div>
